...
| `contextqb check --stale` (drift detector) | ✅     | Compares `tree:`, `routes:`, `decisions:` against sources of truth (`pnpm-workspace.yaml`, `wrangler.jsonc`, `docs/architecture/decisions/`, etc.) and warns on drift. v1 shipped 2026-05-22 as the `@context-qb/cli` workspace package; runnable via `pnpm check:qb`, with the JSON contract under regression test (`pnpm test:check-qb`). Architectural record: [ADR-0014](../../../docs/architecture/decisions/0014-drift-detector-v1.md). Full development trail: [`packages/qb/cli/PUNCHLIST.md`](../cli/PUNCHLIST.md). v1.1 polish work: Gap I (cosmetic `lineOf` quirk), Gap VI (exemption-regex breadth), Gap VII (package-by-name resolution). | P0       |
